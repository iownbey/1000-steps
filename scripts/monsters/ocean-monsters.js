class ManglerFish extends Monster {
  ...
  async attack() {
    if (Math.random() < 0.1) {
      ...
      await new Writer(topWriter, [
        "The Mangler fish's temper has changed.",
      ]).writeAllAsync();
    }
  ...
  async hit(damage) {
    ...
    if (this.h <= 0) return true;
      ...
      if (this.angry) {
        await new Writer(topWriter, [
          "The Mangler Fish lashes out.|It dealt 30 damage.",
        ]).writeAllAsync();
  ...
  magic() {
    if (Math.random() > 0.5) {
      this.toggleAnger();
      return this.angry
        ? "The Mangler fish was enraged by your magics."
        : "The Mangler fish was calmed by your magics.";
  ...
  html(root) {
    var $mangler = $(
      '<canvas style="height:120%;width:120%;left:-10%;" class="mangler"></canvas>'
    );
    this.jobj = $mangler;
...
class Shark extends Monster {
  ...
  async attack() {
    ...
    if (failed === 0) {
      ...
      return {
        text: `Shark munched and dealt ${5 * failed} damage!`,
        damage: 5 * failed,
      };
    }
  }
    ...
    if (this.h <= 0) return true;
      ...
      if (Math.random() < 0.8) {
        if (this.raged) {
          this.unrage();
          await new Writer(topWriter, [
            "You knocked some sense into shark.",
          ]).writeAllAsync();
        } else {
          this.rage();
          await new Writer(topWriter, [
            "Shark is getting mad!",
          ]).writeAllAsync();
        }
      }
      return false;
  ...
  html(root) {
    ...
    this.breatheAnim = new CSSAnimationController($shark, "trollPose").start();
  }
}
...
class Sponge extends Monster {
  constructor() {
    super("Sponge", 30, 2, 1);
    this.flavorText = new NonrepeatingGetter([
      "Sponge douses you with water.",
      "Sponge attacks you with his magical Spongey powers.",
      "Sponge bobs.",
      "A crab jumps out of Sponge at you.",
    ]);
  }
  ...
  attack() {
    ...
    return new Promise((resolve) =>
      resolve({ text: ft + nl + "Sponge dealt {$d} damage.", damage: 2 })
    );
  }
  ...
  inspect() {
    return ["It's a sponge.", "That's it."];
    /*var _this = this;
        return [
...
class Martimer extends Monster {
  ...
  html(root) {
    ...
    this.shakeAnim = new CSSAnimationController($marty, "shake");
    this.shiverAnim = new CSSAnimationController($marty, "shiver");
  }
